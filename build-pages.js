...
const JS = `
  ...
  function showNewVideoIndicators() {
    ...
    pending.forEach(p => {
      ...
      // Not watched — show indicator below card
      const indicator = document.createElement('div');
      indicator.className = 'new-video-indicator';
      indicator.innerHTML = \`<span class="indicator-badge">🆕 NEW</span> <strong>\${p.creator}</strong>: \${p.title} · <span class="indicator-duration">\${p.duration || ''}</span>\`;
